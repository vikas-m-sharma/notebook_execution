...
class CreateJobRequest(BaseModel):
    ...
    name: str = Field(..., description="Unique job identifier name (e.g. 'daily-sales-report').")
    ...
    schedule_type: str = Field("ONE_TIME", description="Scheduling strategy (ONE_TIME or CRON).")
    cron_expression: Optional[str] = Field(None, description="5-field cron expression (e.g. '0 2 * * *').")
    timezone: str = Field("UTC", description="Explicit timezone identifier (e.g. 'Asia/Kolkata', 'UTC').")
    concurrency_policy: str = Field("PREVENT_OVERLAP", description="Concurrency policy (ALLOW_CONCURRENT or PREVENT_OVERLAP).")
    max_retries: int = Field(0, ge=0, description="Maximum execution retry attempts.")
    retry_delay_seconds: int = Field(60, ge=0, description="Delay in seconds between retries.")
...
class UpdateJobRequest(BaseModel):
    ...
    schedule_type: Optional[str] = Field(None, description="Updated schedule type.")
    ...
    concurrency_policy: Optional[str] = Field(None, description="Updated concurrency policy.")
    max_retries: Optional[int] = Field(None, ge=0, description="Updated max retries.")
    retry_delay_seconds: Optional[int] = Field(None, ge=0, description="Updated retry delay in seconds.")
